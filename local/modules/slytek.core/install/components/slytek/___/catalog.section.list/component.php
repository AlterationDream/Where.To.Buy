...
if($arParams['SECTION_FILTER'] && $GLOBALS[$arParams['SECTION_FILTER']] && is_array($GLOBALS[$arParams['SECTION_FILTER']])){
	$arSectionFilter=$GLOBALS[$arParams['SECTION_FILTER']];
}

if($arParams['ELEMENT_FILTER'] && $GLOBALS[$arParams['ELEMENT_FILTER']] && is_array($GLOBALS[$arParams['ELEMENT_FILTER']])){
	$arElementFilter=$GLOBALS[$arParams['ELEMENT_FILTER']];
}

$filter_id=serialize($arSectionFilter).serialize($arElementFilter);

/*************************************************************************
			Work with cache
*************************************************************************/
if($this->startResultCache(false, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()).$filter_id))
{
	...
	if($arSectionFilter){
		$arFilter=array_merge($arFilter, $arSectionFilter);
	}
	$custom_get_count=false;
	if($arElementFilter && $arParams["COUNT_ELEMENTS"]){
		$arParams["COUNT_ELEMENTS"]=false;
		$custom_get_count=true;
		$arElementFilter=array_merge(array(
			'IBLOCK_ID'=>$arParams['IBLOCK_ID'],
			'ACTIVE'=>'Y',
	...
	{
		if($custom_get_count){
			$arSection['ELEMENT_CNT'] = CIBlockElement::GetList(Array(), array_merge($arElementFilter, array(
