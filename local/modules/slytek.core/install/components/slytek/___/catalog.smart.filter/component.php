...
if($this->StartResultCache(false, 'v5'.($arParams["CACHE_GROUPS"]? $USER->GetGroups(): false)))
{
	...
	if(!empty($arResult["ITEMS"]))
	{
		if ($this->facet->isValid())
		{
			...
			if($GLOBALS[$arParams['FILTER_NAME']])$arResult["FACET_FILTER"]=array_merge($arResult["FACET_FILTER"], $GLOBALS[$arParams['FILTER_NAME']]);
		...
		else
		{
			...
			if($GLOBALS[$arParams['FILTER_NAME']])$arElementFilter=array_merge($arElementFilter, $GLOBALS[$arParams['FILTER_NAME']]);
			$arElements = array();
...
if ($arResult["JS_FILTER_PARAMS"]["SEF_SET_FILTER_URL"])
{
	...
	$arResult["FILTER_AJAX_URL"] = (CHTTP::urlAddParams($arResult["FILTER_URL"], array(
...
else
{
	...
	$arResult["FILTER_URL"] = (CHTTP::urlAddParams($clearURL, $paramsToAdd, array(
		"skip_empty" => true,
		"encode" => true,
	)));

	$arResult["FILTER_AJAX_URL"] =(CHTTP::urlAddParams($clearURL, $paramsToAdd + array(
		"bxajaxid" => $_GET["bxajaxid"],
	), array(
		"skip_empty" => true,
		"encode" => true,
