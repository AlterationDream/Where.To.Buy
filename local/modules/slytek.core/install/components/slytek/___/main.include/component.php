...
if(!CModule::includeModule('slytek.settings'))return;
/************************************************************************************************************/
...
if($arParams['AREA_FILE_SHOW']=='prop' && $arParams['PROPERTY_ID'] && CModule::includeModule('slytek.settings')){
	$arResult['CONTENT']=\Slytek\Settings::getOption(htmlspecialchars_decode($arParams['PROPERTY_ID']), $arParams['SITE_ID']);
...
if($arParams['AREA_FILE_SHOW']!='prop' && $APPLICATION->GetShowIncludeAreas())
{
			...
			if ($bCanEdit)
			{
				...
				if($USER->CanDoOperation('edit_php'))
				{
					$arMenu[] = array(
						...
						"ICON" => "panel-edit-php",
						"TEXT"=>GetMessage("SLYTEK_comp_include_edit_php"),
						"TITLE" => GetMessage("SLYTEK_INCLUDE_AREA_EDIT_".$arParams["AREA_FILE_SHOW"]."_NOEDITOR"),
						);
				}
				$arIcons = array(
					array(
						...
						"TITLE"=>GetMessage("SLYTEK_comp_include_edit"),
						"ALT" => GetMessage("SLYTEK_INCLUDE_AREA_EDIT_".$arParams["AREA_FILE_SHOW"]),
						"MENU" => $arMenu,
						),
					);
			}
			...
			if ($sFilePath != $sFilePathTMP && $bCanAdd)
			{
				...
				if($USER->CanDoOperation('edit_php'))
				{
					$arMenu[] = array(
						...
						"ICON" => "panel-edit-php",
						"TEXT"	=> GetMessage("SLYTEK_comp_include_add_php"),
						"TITLE" => GetMessage("SLYTEK_INCLUDE_AREA_ADD_".$arParams["AREA_FILE_SHOW"]."_NOEDITOR"),
						);
				}
				$arIcons[] = array(
					"URL" => 'javascript:'.$APPLICATION->GetPopupLink(
					...
					"TITLE" => GetMessage("SLYTEK_comp_include_add"),
					"ALT" => GetMessage("SLYTEK_INCLUDE_AREA_ADD_".$arParams["AREA_FILE_SHOW"]),
					"MENU" => $arMenu,
					);
			}
		}
		elseif ($bCanAdd)
		{
			...
			if($USER->CanDoOperation('edit_php'))
			{
				$arMenu[] = array(
					...
					"ICON" => "panel-edit-php",
					"TEXT" => GetMessage("SLYTEK_comp_include_add1_php"),
					"TITLE" => GetMessage("SLYTEK_INCLUDE_AREA_ADD_".$arParams["AREA_FILE_SHOW"]."_NOEDITOR"),
					);
			}
			$arIcons = array(
				array(
					...
					"DEFAULT" => $APPLICATION->GetPublicShowMode() != 'configure',
					"ICON" => "bx-context-toolbar-create-icon",
					"TITLE" => GetMessage("SLYTEK_comp_include_add1"),
					"ALT" => GetMessage("SLYTEK_INCLUDE_AREA_ADD_".$arParams["AREA_FILE_SHOW"]),
					"MENU" => $arMenu,
					),
				);
...
if ($bFileFound || $arResult['CONTENT'])
{
	if($arResult['CONTENT']){
		...
		if(!$settings_prop)$arResult["FILE"] = $io->GetPhysicalName($_SERVER["DOCUMENT_ROOT"].$sFilePath.$sFileName);
		$this->IncludeComponentTemplate();
