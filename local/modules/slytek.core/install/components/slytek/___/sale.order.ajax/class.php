...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	protected function obtainTotal()
	{
		...
		$arResult['TO_PAY_PRICE']=$this->GetRemaining($this->order);
		$arResult['TO_PAY_PRICE_FORMATED']=SaleFormatCurrency($arResult['TO_PAY_PRICE'], $this->order->getCurrency());
	 ...
	 * @throws Main\ObjectNotFoundException
	 */
	protected function GetRemaining(Order $order){
		...
		foreach ($basketItems as $item) {
			...
			$ib = \CIBlockElement::GetIBlockByID($pid);
			if(in_array($ib, array(\Paris::id['tours'], \Paris::id['offers'], \Paris::id['transfer']))){
				$price = $item->getPrice();
				if($price<$this->arParams['PREPAY_SIZE'] || intval($this->arParams['PREPAY_SIZE'])==0){
					$price = $this->arParams['PREPAY_SIZE'];
				}
				else{
					$price = $this->arParams['PREPAY_SIZE']*$item->getQuantity();
				}
				$remain+=$price;
			}else{
				$remain+= $item->getFinalPrice();
			}
		}
		return $remain;
	}
	protected function recalculatePayment(Order $order)
	{
		...
		if($this->arParams['PREPAY_SIZE']>0){
			$remainingSum = $this->GetRemaining($order);
		}
		if ($remainingSum > 0 || $order->getPrice() == 0)
		{
			$paymentCollection = $order->getPaymentCollection();
	...
	public function executeComponent()
	{
		...
		$this->arParams['PREPAY_SIZE'] = intval(\Slytek\Settings::get('prepay'));
